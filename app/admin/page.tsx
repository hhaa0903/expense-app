...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (authed && adminTab === 'users') fetchUsers()
  }, [authed, adminTab])
  ...
  const toggleUserActive = async (user: User) => {
    ...
    const label = next ? '恢復' : '停用'
    if (!confirm(`確定要${label}「${user.name}」的帳號嗎？`)) return
    const { error } = await supabase
      .from('users')
  ...
  const ExpenseCard = ({ e, showActions }: { e: Expense; showActions: boolean }) => {
    ...
    const borderColor = showActions
      ? isCollection ? 'border-yellow-100' : 'border-orange-100'
      : 'border-gray-100'
    ...
    return (
      <div className={`bg-white rounded-xl border ${borderColor} p-4 ${!showActions ? 'opacity-60' : ''}`}>
        <div className="flex justify-between items-start mb-2">
          <div>
            {isCollection
              ...
              : <span className="text-sm font-medium text-gray-800">{categoryMap[e.category] || e.category}</span>
            }
            <span className="text-xs text-gray-400 ml-2">{e.expense_date}</span>
          </div>
          <span className={`text-lg font-bold ${isCollection ? 'text-yellow-600' : 'text-orange-500'}`}>
        ...
        {e.erp_ref_no && (
          <p className="text-xs text-green-500 mb-2">#{e.erp_ref_no}</p>
        )}
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <p className="text-sm text-gray-400 mb-5">管理所有費用申請</p>
        ...
        {adminTab === 'users' && (
          <div>
            ...
            {!usersLoading && users.length === 0 && (
              <p className="text-sm text-gray-400">目前沒有員工帳號</p>
            )}
